...
ADCサービスのAPI経由で利用することも可能です。その場合、curlを用いてAPIの利用方法を確認できます。
...
#### 設定ファイル
設定ファイル`adcclient.json`が、ホームディレクトリに作成されます。

オプション`--username`や`--URL`で指定した値は、設定ファイルに保存され、以後、デフォルト値として利用されるようになります。

Webのクッキーが設定ファイルに保存され、loginに成功した後は、ログイン状態が継続します。

設定ファイルにパスワードは保存されません。
