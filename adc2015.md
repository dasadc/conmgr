...
- これはクライアントの死活監視のための機能です。コンテスト本番中は、5分に最低1回は、サーバと通信することを推奨します。サーバは、クライアントからアクセス無い時間がつづくと、クライアントに異常が発生しているのでは？と認識します。
...
- 過去2日のログを見たいときは、`adccli get-user-log 2 days`です。
